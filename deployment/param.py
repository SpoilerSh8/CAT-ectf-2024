...
with open("hello.h", "w") as file:
        file.write("\t --- Colombe Academy of Technology ---\n")
        file.write("From Dakar, Senegal\n")
        file.write("Participating as 2024-CAT\n")
        file.close()
...
if(params[0] == "ectf_build_ap"):
    ...
    params = shlex.split(command)
    #--- execute la commande build AP pour lui ici
    for i in range(1,component_cnt):
        if(params[0] == "ectf_build_comp"):
            ...
            # Enregistrer les paramètres chiffrés dans le fichier
            save_Comp_params()
            #--- execute la commande build component ici pour lui
